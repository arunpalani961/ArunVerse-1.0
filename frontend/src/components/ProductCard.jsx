...
const ProductCard = ({ product }) => {
  ...
  return (
    <div className="group h-full bg-white rounded-md overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-200 hover:border-[#ff9900]">
      {/* Image Container */}
      <div className="relative h-64 overflow-hidden bg-gray-100 cursor-pointer" onClick={handleClick}>
        <img
          src={product.image}
          alt={product.title}
      ...
      <div className="p-6 flex flex-col h-full justify-between">
        ...
        <div>
          <div className="flex items-start justify-between gap-2 mb-3">
            <h2 className="text-lg font-bold text-[#111827] line-clamp-2">
              {product.title}
            </h2>
        ...
        <div className="mt-4 pt-4 border-t border-gray-200 flex items-center justify-between">
          ...
          <button
            onClick={handleClick}
            className="px-4 py-2 bg-[#ffd814] text-[#111827] rounded-full hover:bg-[#f7ca00] transition-all duration-300 text-sm font-semibold border border-[#fcd200]"
          >
            View Product
          </button>
        </div>
