    ...
    <nav className="sticky top-0 z-50 bg-[#131921] border-b border-[#febd69]/40 shadow-md">
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-3 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center justify-end gap-2 sm:gap-4">
          {isAdmin && (
            <>
              <Link to="/admin" className="min-h-10 px-3 sm:px-4 py-2 text-sm sm:text-base text-white hover:text-[#febd69] transition">
                Dashboard
              </Link>
              <button
                onClick={handleLogout}
                className="min-h-10 px-3 sm:px-4 py-2 bg-[#febd69] text-[#111827] rounded hover:bg-[#f3a847] transition text-sm sm:text-base font-semibold"
              >
                Logout
              </button>
            </>
